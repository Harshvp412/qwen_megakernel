...
def main():
    ...
    max_steps = min(12, len(REF_GENERATED))  # check first 12 steps
    ...
    for gen_step in range(max_steps):
        ...
        if not match:
            print(f"         HF top3: {[(int(hf_top3.indices[i]), float(hf_top3.values[i])) for i in range(3)]}")
            print(f"         MK top3: {[(int(mk_top3.indices[i]), float(mk_top3.values[i])) for i in range(3)]}")
            # Logit at expected token
            if expected is not None:
                print(f"         Logit at expected {expected}: HF={hf_logits[expected]:.4f}  MK={mk_logits[expected]:.4f}")
    ...
    if first_divergence is not None:
        print(f"First divergence at gen_step {first_divergence} (next token index {first_divergence}).")
        print(f"  → Bug is introduced when processing token at position {len(prompt_ids) + first_divergence - 1}")
        print(f"  → Check KV write at that position or attention over 0..{len(prompt_ids) + first_divergence - 1}")
    else:
        print("No divergence in the steps checked.")
    return 0
