...
If the reference file is missing, `compare_tokens.py` will generate it automatically on first run (same machine). To regenerate or use a different model:
...
- **Reference file:** `parity_reference_output.json` is used by `compare_tokens.py`. If missing, it is generated automatically on first run. To regenerate: `python parity_reference.py --model Qwen/Qwen3-0.6B`
