...
async def test_tts_service_run_tts(write_wav: str | None) -> bool:
    ...
    if not frames:
        print("✗ No frames yielded")
        return False

    if not isinstance(frames[0], TTSStartedFrame):
        print(f"✗ First frame should be TTSStartedFrame, got {type(frames[0]).__name__}")
        return False
    if not isinstance(frames[-1], TTSStoppedFrame):
        print(f"✗ Last frame should be TTSStoppedFrame, got {type(frames[-1]).__name__}")
        return False

    audio_frames = [f for f in frames if isinstance(f, TTSAudioRawFrame)]
    if not audio_frames:
        print("✗ No TTSAudioRawFrame yielded")
        return False

    total_bytes = sum(len(f.audio) for f in audio_frames)
    if total_bytes == 0:
        print("✗ All audio frames are empty")
        return False
